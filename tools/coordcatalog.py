...
class CatalogData():
	...
	def calculate(self):
		...
		for polygon in self.list_contours:
			...
			table += u'<TABLE CELLSPACING=\"0\" COLS=\"5\" BORDER=\"0\"><COLGROUP SPAN=\"5\" WIDTH=\"86\"></COLGROUP>{0}</TABLE>'
			empty = u'<TD STYLE=\"border-top: 1px solid #000000; border-bottom: 1px solid #000000; border-left: 1px solid #000000; border-right: 1px solid #000000\" HEIGHT=\"17\" ALIGN=\"CENTER\">{0}</TD>'
			...
			self.catalog += u'<TR>{0}</TR>'.format(header)
		for ring in polygon:
			iter_n = 1
			for point in ring:
